...
![IMG_20230323_211639](IMG_20230323_211639.jpg)
...
### 使用方法
配置随机测试评论池，将会随机抽取一个用于评论被ban时回复该评论测试评论状态以及评论区是否戒严。一行一个，最少4个，如果你没配置默认为古诗3首。  
...
```
为什么发送后要等待2秒再检查（流程图中略）？因为发布之后，系统需要一定的时间来处理评论，发送了立即获取评论列表导致获取到的最新评论没有该评论但是评论其实没有问题的，从而导致误判。
## 检查评论区
```mermaid
graph TB
    A[在该评论区发布测试评论]  --> B(无账号环境下获取最新评论列表) --> C{是否找到该评论}
    C --找到了--> Z(删除测试评论)--> D[评论区正常] --> J[测试是否仅在此评论区被ban] --> K(在你的评论区发送一样内容的评论) --> L{以上方法检测是否正常显示}
    C --没找到--> E[评论区被戒严] --> F(回复该评论)
    F --"提示“已经被删除了”" --> AC(删除测试评论) --> G[评论默认立即删除]
    F --回复成功--> H(删除测试评论及回复) --> Y[评论默认shadowban] 
    L --显示正常--> AA(删除测试评论) --> X[仅在此评论区被ban]
    L --显示异常--> AB(删除测试评论) --> V[在其他评论区也被ban]
...
发送申诉后，人机不会看你的申诉理由，有很多时候你遇到的是人机，少许时候是真人，真人才会去理解申诉理由。  

人机申诉会通过你发的一系列被ban状态的评论，也就是如果你发了多条，他会全部恢复，无论评论是多久前发的。他不会看你的申诉理由，你在申述理由里填要指定恢复那条是没用的，除了人工会看。在shadowban评论上回复的任何评论都属于shadowban评论，一样会加入待申诉列表里。还有一个特性（bug？）是：你删除掉了shadowban评论，但是申诉过后依然会系统通知你“您的评论申诉已处理”，然后下面跟着你评论的略写，不过都是“无法恢复”，也从来没有见过这些自己删的恢复成功的案例（不排除可能成功）。  
