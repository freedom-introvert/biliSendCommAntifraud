...
```
为什么发送后要等待一段时间再检查（流程图中略）？因为发布之后，系统需要一定的时间来处理评论，发送了立即获取评论列表导致获取到的最新评论没有该评论但是评论其实没有问题的，从而导致误判。当然你可以设置发送后要等待多久。  
### 新检查方法   
...
```mermaid
graph TB
    10[评论发送] --> 20(无账号下翻找评论列表)
    20 --找到了--> 400[评论正常显示]
    20 --没找到 --> 60(有账号获取该评论的回复列表) --> 70(获取成功) --> 80[评论被shadowban]
    60 --> 90(提示已经被删除了) --> 100(回复该评论) --> 110(还是提示已经被删除了) --> 105[评论被系统秒删]
